...
from src.agent.prompt.cot_prompt import COT_COMPLETION_PROMPT_TEMPLATES
...
class Agent:
    ...
    def _construct_prompt(self, query: str, agent_scratchpad: str) -> str:
        """构造提示模板，使用缓存优化工具描述生成"""
    ...
    def _parse_action(self, response_text: str) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                "Thought": f"哈哈，好像有些不对劲 {str(e)}",
                "Action": {"action": "Final Answer", "action_input": response_text},
        ...
        try:
            ...
            while iteration_count < self.max_iterations:
                ...
                try:
                    ...
                    result_dict = self._parse_action(model_response)
                    action_dict = result_dict.get("Action", {})
