...
class PythonExecuteNode(BaseNode):
    ...
    async def execute(self, params: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if not isinstance(variables, dict):
            raise ValueError("variables参数必须为字典类型")
    ...
    async def agent_execute(self, params: Dict[str, Any]) -> Dict[str, Any]:
        ...
        combined_text = (
            f"执行函数：{execution_result.get('function_name', 'N/A')}\n"
            f"返回结果：{str(execution_result.get('result', 'N/A'))}\n"
            f"标准输出：{execution_result.get('stdout', '无')}\n"
            f"标准错误：{execution_result.get('stderr', '无')}\n"
            f"错误信息：{execution_result.get('error', '无')}"
        )
